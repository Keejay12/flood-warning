...
df = pd.read_csv("waterlevels.csv")

# Check for required columns
if "water_level" not in df.columns or "humidity" not in df.columns or "temperature" not in df.columns:
    st.error("The required columns ('water_level', 'humidity', 'temperature') are not present in the DataFrame.")
    st.stop()
...
with placeholder.container():
    ...
    # Allow users to select specific columns to display
    selected_columns = st.multiselect("Select Columns", df.columns, default=list(df.columns))
    st.dataframe(filtered_df[selected_columns], use_container_width=True) # Adjust width to fit the screen
